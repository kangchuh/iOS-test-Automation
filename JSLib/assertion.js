...
MemberInfo = {
	 /*
	 	please write your member in here.
	 */
	version:"iPhone  test automation."
};
/**
...
/**
 * Asserts that the given expression is false and otherwise throws an exception
 * with a default message, or the optional +message+ parameter
 */
...
function assertNull(thingie, message) {
  var defMessage = "期盼值 == null , 但是实际值 <" + thingie + ">"; 
  // TODO: string-matching on UIAElementNil makes my tummy feel bad. Fix it.
  return assertTrue(thingie === null || thingie.toString() == "[object UIAElementNil]",
...
/**
 * 这里校验文本，控件的 显示文本或者value值是否与期望的相匹配。
 * 不匹配则抛出异常，用例运行失败，匹配则写入日志信息并继续运行下一步骤
 */
 ...
 *该事件返回true的时候，不触发默认的自动处理事件，在这里我们也加入了相应的日志，记录对话框的出现
 *注意事项：
 ...
 *onAlert事件完成之前，动作被阻塞，无法操作弹出窗口。完成onAlert事件后继续玩下运行。
 */
